...
export default function TakeExamPage() {
  ...
  if (isBlocked) return <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center text-center p-8"><ShieldAlert className="w-20 h-20 text-primary mb-6 animate-pulse" /><h2 className="text-3xl font-black text-white">🚨 محتوى محمي</h2><p className="text-primary font-bold mt-2">يمنع تصوير الشاشة أو الخروج من الصفحة.</p></div>;

  if (finishedResult) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 text-right">
      <Card className="w-full max-w-xl bg-card border-primary/10 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in">
        <div className={cn("h-3", finishedResult.isSuccess ? "bg-accent" : "bg-destructive")} />
        <CardContent className="p-12 text-center space-y-10">
           <div className={cn("w-24 h-24 rounded-full flex items-center justify-center mx-auto", finishedResult.isSuccess ? "bg-accent/10 text-accent" : "bg-destructive/10 text-destructive")}>
           ...
           {exam?.allowInstantResultsDisplay ? (
              <div className="grid grid-cols-2 gap-6">
                <div className="p-8 bg-secondary/30 rounded-3xl border border-white/5"><p className="text-5xl font-black text-primary">{finishedResult.score}%</p><p className="text-[10px] font-bold text-muted-foreground mt-2">النسبة</p></div>
                <div className="p-8 bg-secondary/30 rounded-3xl border border-white/5"><p className="text-5xl font-black text-accent">{finishedResult.points}/{finishedResult.total}</p><p className="text-[10px] font-bold text-muted-foreground mt-2">النقاط</p></div>
              </div>
           ) : (
             <p className="text-muted-foreground font-bold">تم استلام إجاباتك، النتائج ستظهر فور اعتماد المدرس.</p>
           )}
           <Link href="/student/dashboard" className="block"><Button className="w-full h-16 bg-primary font-black rounded-2xl text-xl">العودة للوحة التحكم</Button></Link>
        </CardContent>
      </Card>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background pb-24 text-right">
      <div className="sticky top-0 z-50 bg-card/80 backdrop-blur-md border-b w-full flex items-center justify-between p-4 px-6">
          <div className="text-lg font-bold bg-primary/10 px-4 py-2 rounded-xl text-primary flex items-center gap-2"><Clock className="w-4 h-4" /> {timeLeft !== null ? `${Math.floor(timeLeft/60)}:${(timeLeft%60).toString().padStart(2,'0')}` : '--:--'}</div>
...
function MCQOptions({ courseId, examId, qId, selected, onSelect }: any) {
  ...
  return (
    <div className="grid gap-4">
      {options?.map(o => (
        <div key={o.id} onClick={() => onSelect(o.id)} className={cn("flex flex-row-reverse items-center gap-4 p-6 border-2 rounded-3xl cursor-pointer transition-all", selected === o.id ? "border-primary bg-primary/5 shadow-inner" : "border-white/5 hover:bg-white/5")}>
           <div className={cn("w-6 h-6 rounded-full border-2", selected === o.id ? "border-primary bg-primary" : "border-muted")} />
