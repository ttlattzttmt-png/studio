...
export default function TakeExamPage() {
  ...
  const handleSubmit = async () => {
    if (isSubmitting || !questions || !user) return;
    setIsSubmitting(true);
    try {
      const studentProfile = await getDocs(query(collection(firestore!, 'students'), where('__name__', '==', user.uid)));
      const name = studentProfile.docs[0]?.data().name || 'طالب مجهول';
  ...
  if (isBlocked) return <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center text-center p-8 select-none"><ShieldAlert className="w-20 h-20 text-primary mb-6 animate-pulse" /><h2 className="text-3xl font-black text-white">🚨 محتوى محمي</h2><p className="text-primary font-bold mt-2">يمنع منعاً باتاً تصوير الشاشة.</p></div>;

  if (finishedResult) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <Card className="w-full max-w-xl bg-card border-primary/10 rounded-[2.5rem] overflow-hidden shadow-2xl text-right">
        <div className="h-3 bg-accent" />
        <CardContent className="p-12 text-center space-y-10">
           <div className="w-24 h-24 rounded-full bg-accent/10 flex items-center justify-center mx-auto text-accent shadow-lg"><Trophy className="w-12 h-12" /></div>
           <h1 className="text-4xl font-black">أحسنت يا بطل!</h1>
           <div className="grid grid-cols-2 gap-6">
              <div className="p-8 bg-secondary/30 rounded-3xl border border-white/5"><p className="text-5xl font-black text-primary">{finishedResult.score}%</p><p className="text-[10px] font-bold text-muted-foreground mt-2 uppercase">النسبة</p></div>
              <div className="p-8 bg-secondary/30 rounded-3xl border border-white/5"><p className="text-5xl font-black text-accent">{finishedResult.points}/{finishedResult.total}</p><p className="text-[10px] font-bold text-muted-foreground mt-2 uppercase">النقاط</p></div>
           </div>
           <Link href="/student/dashboard" className="block"><Button className="w-full h-16 bg-primary text-primary-foreground font-black rounded-2xl text-xl">العودة للرئيسية</Button></Link>
        </CardContent>
      </Card>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background pb-24 select-none">
      <div className="sticky top-0 z-50 bg-card/80 backdrop-blur-md border-b w-full flex items-center justify-between p-4">
          <div className="text-lg font-bold bg-primary/10 px-4 py-2 rounded-xl text-primary flex items-center gap-2"><Clock className="w-4 h-4" /> {timeLeft ? `${Math.floor(timeLeft/60)}:${(timeLeft%60).toString().padStart(2,'0')}` : '--:--'}</div>
          <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-primary font-black px-8 h-12 rounded-xl shadow-lg">إنهاء</Button>
      </div>
      <main className="container mx-auto p-4 max-w-3xl pt-10">
        <Card className="bg-card border-primary/10 rounded-[2.5rem] p-10 text-right space-y-8">
           ...
           <div className="flex justify-between pt-8">
              ...
              <Button variant="outline" disabled={activeQuestionIndex === questions.length - 1} onClick={() => setActiveQuestionIndex(p => p + 1)} className="h-12 w-32 rounded-xl">التالي</Button>
           </div>
        </Card>
      </main>
    </div>
